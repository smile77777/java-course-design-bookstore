...
<div id="main">
    ...
    <div class="shopping_list_border">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" id="myTableProduct">
            <%Map<String,List<book>> map= (Map<String, List<book>>) session.getAttribute("bookmap");
                Set<String> keys = map.keySet();
            for(String str:keys){
            %>
            <tr class="shopping_product_list" id="shoppingProduct_01">
                <td class="shopping_product_list_1"><a href="#" class="blue"><%=map.get(str).get(0).getBookname()%></a></td>
                <td class="shopping_product_list_2"><label>189</label></td>
                <td class="shopping_product_list_3">￥<label><%=map.get(str).get(0).getBookprice()%></label></td>
                <td class="shopping_product_list_4">￥<label><%=map.get(str).get(0).getBookpriced()%></label></td>
                <td class="shopping_product_list_5"><input type="text" value="1"/></td>
                <td class="shopping_product_list_6"><a href="bookservlet?bookno=<%=map.get(str).get(0).getBookid()%>&op=remove" onclick="javascript:void(0)" class="blue">删除</a></td>
            </tr>
            <%}%>
        </table>
        <div class="shopping_list_end">
            <div><a id="removeAllProduct"  href="bookservlet?op=rem" onclick="javascript:void(0)">清空购物车</a></div>
            <ul>
